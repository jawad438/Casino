...
const SYMBOLS = ['💎', '🍒', '🍋', '🔔', '⭐️', '7️⃣', '💰', '🍀', '🍎', '🍇'];
// We use 5 sets of symbols to ensure plenty of room for a long spin animation
...
export const Slots: React.FC<{ balance: number; updateBalance: (a: number) => void }> = ({ balance, updateBalance }) => {
  ...
  const [reels, setReels] = useState([0, 0, 0]); // Indices relative to SYMBOLS array
  ...
  const spin = () => {
    ...
    // Final visible results (base index 0-9)
    const results = [
    ...
    // Target the symbols in the 4th set (indices 30-39) for a long forward spin
    const spinTargets = results.map(r => r + SYMBOLS.length * 3);
    setReels(spinTargets);

    // 2000ms matches the CSS transition duration
    setTimeout(() => {
      // 1. Instantly disable transition
      setUseTransition(false);
      
      // 2. Snap to the equivalent position in the 1st set (indices 0-9)
      // Since useTransition is false, this happens instantly (no "roll back")
      setReels(results);
      ...
      <div className="flex gap-4 p-8 bg-zinc-950 rounded-[3rem] border-8 border-zinc-900 shadow-[0_0_50px_rgba(0,0,0,0.5),inset_0_0_20px_rgba(255,255,255,0.05)] relative overflow-hidden">
        ...
        {reels.map((pos, i) => (
          <div key={i} className="w-24 h-40 sm:w-32 sm:h-56 bg-zinc-900 rounded-3xl flex flex-col items-center overflow-hidden border border-white/5 shadow-inner relative z-10">
            <div 
              className={`flex flex-col ${useTransition ? 'transition-transform duration-[2000ms] [transition-timing-function:cubic-bezier(0.45,0.05,0.2,1)]' : 'transition-none'}`}
              style={{ transform: getTransform(pos) }}
            >
              {REEL_STRIP.map((s, idx) => (
                <div key={idx} className="h-40 sm:h-56 flex items-center justify-center text-5xl sm:text-7xl shrink-0 select-none drop-shadow-2xl">
                  {s}
                </div>
              ))}
            </div>
          ...
          <div className="space-y-2 text-center">
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={spinning}
                className="w-full bg-zinc-950 border border-white/10 rounded-2xl py-4 pl-10 pr-4 mono text-white font-bold text-xl text-center focus:outline-none focus:ring-4 focus:ring-yellow-500/20 transition-all"
              />
            </div>
          </div>
          <button
            ...
            className="w-full py-6 bg-gradient-to-r from-yellow-500 via-amber-600 to-orange-600 hover:from-yellow-400 hover:to-orange-500 disabled:opacity-50 text-white font-black rounded-2xl shadow-2xl shadow-orange-600/30 uppercase tracking-[0.3em] text-2xl transition-all active:scale-95"
          >
            {spinning ? 'SPINNING...' : 'JACKPOT SPIN'}
          </button>
        </div>
        <div className="h-12 flex items-center">
          {message && (
            <div className={`text-3xl font-black uppercase tracking-widest text-neon animate-in zoom-in duration-300 ${message.includes('WIN') ? 'text-yellow-400 drop-shadow-[0_0_10px_rgba(250,204,21,0.5)]' : 'text-zinc-600'}`}>
              {message}
            </div>
          )}
        </div>
