          ...
          return (
            <div key={i} className="aspect-square relative transition-transform duration-500" style={{ transformStyle: 'preserve-3d', transform: isRevealed || showCheat ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
              ...
              <button
                ...
                className="absolute inset-0 bg-zinc-800 hover:bg-zinc-700 rounded-xl border border-white/5 flex items-center justify-center"
                style={{ backfaceVisibility: 'hidden' }}
              />
              
              {/* Back Face */}
              <div 
                className={`absolute inset-0 rounded-xl flex items-center justify-center text-3xl shadow-inner ${isMine ? 'bg-red-500 shadow-red-950/50' : 'bg-emerald-600 shadow-emerald-900/50'}`}
      ...
      <div className="w-full md:w-80 flex flex-col gap-5 bg-zinc-950 p-6 rounded-[2.5rem] border border-white/10 shadow-2xl">
        <div className="flex flex-col gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              disabled={gameState === 'playing'}
              className="w-full bg-zinc-900 border border-white/10 rounded-2xl py-3 pl-9 pr-4 mono text-white font-bold focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all"
            />
          </div>
          <div className="flex gap-1.5 mt-1">
            {[10, 50, 100].map(v => (
              <button 
                ...
                onClick={() => setBet(v)} 
                disabled={gameState === 'playing'}
                className={`flex-1 py-2 rounded-lg text-[10px] font-black transition-all ${bet === v ? 'bg-zinc-700 text-white' : 'bg-zinc-900 text-zinc-600 hover:text-zinc-300'}`}
              >
                ${v}
              </button>
            ))}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex bg-zinc-900 rounded-xl p-1 border border-white/5">
            {[1, 3, 5, 13].map(v => (
              <button key={v} onClick={() => setMineCount(v)} disabled={gameState === 'playing'} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${mineCount === v ? 'bg-emerald-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}>{v}</button>
            ))}
          </div>
        </div>
        ...
        {gameState === 'playing' ? (
          ...
          <button 
            ...
            className="py-6 bg-indigo-600 text-white font-black rounded-2xl hover:bg-indigo-500 mt-auto uppercase tracking-[0.2em] shadow-xl shadow-indigo-600/20 text-lg transition-all active:scale-95"
          >
            ENTER GRID
          </button>
        )}
