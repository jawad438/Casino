...
export const Roulette: React.FC<{ balance: number; updateBalance: (a: number) => void }> = ({ balance, updateBalance }) => {
  ...
  const spin = () => {
    ...
    const targetRotation = rotation + (spins * 360) - (outcomeIndex * segmentAngle);
  ...
  return (
    <div className="flex flex-col items-center gap-10">
            ...
            return (
              <g key={num}>
                <path 
                  ...
                  className={`${getNumColor(num)} stroke-zinc-700/30 stroke-[0.5]`}
                />
                <text
                  x="100"
                  y="25"
                  transform={`rotate(${angle + (180 / 37)}, 100, 100)`}
                  textAnchor="middle"
                  className="fill-white font-black text-[6px] mono pointer-events-none"
                >
                  {num}
                </text>
              </g>
      ...
      <div className="w-full space-y-6">
        <div className="grid grid-cols-6 sm:grid-cols-12 gap-1.5 p-3 bg-zinc-950 rounded-2xl border border-white/5">
          {Array.from({ length: 37 }).map((_, i) => (
            <button
              ...
              className={`aspect-square flex items-center justify-center rounded-lg text-[10px] font-black transition-all ${selectedBet === i ? 'ring-2 ring-yellow-500 scale-105 z-10 shadow-lg' : 'hover:opacity-80'} ${i === 0 ? 'bg-green-600' : RED_NUMBERS.includes(i) ? 'bg-red-600' : 'bg-zinc-800'}`}
            >
              {i}
            </button>
          ))}
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          {[
            ...
            { id: 'black', color: 'bg-zinc-800', label: 'BLACK' },
            { id: 'even', color: 'bg-zinc-700', label: 'EVEN' },
            { id: 'odd', color: 'bg-zinc-700', label: 'ODD' }
          ].map(betType => (
            <button
              key={betType.id}
              onClick={() => setSelectedBet(betType.id)}
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black tracking-[0.2em] border transition-all ${selectedBet === betType.id ? 'ring-2 ring-yellow-500 scale-105' : 'border-white/5 hover:border-white/20'} ${betType.color}`}
            >
              {betType.label}
            </button>
          ))}
        ...
        <div className="flex-grow w-full space-y-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              disabled={spinning}
              className="w-full bg-zinc-950 border border-white/5 rounded-2xl py-3 pl-9 pr-4 mono text-white font-bold focus:outline-none focus:ring-2 focus:ring-red-500/50"
            />
          </div>
        </div>
        <button
          onClick={spin}
          disabled={spinning || !selectedBet || balance < bet || bet <= 0}
          className="w-full sm:w-auto px-10 py-5 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white font-black rounded-2xl shadow-lg shadow-red-600/30 uppercase tracking-widest text-sm self-end"
        >
      ...
      {message && (
        <div className={`text-xl font-black uppercase tracking-widest animate-in fade-in slide-in-from-bottom-2 duration-300 ${message.includes('WINNER') ? 'text-green-400' : 'text-zinc-500'}`}>
          {message}
        </div>
      )}
    </div>
